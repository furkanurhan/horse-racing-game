...
const currentRoundIndex = computed(() => store.state.racing.currentRoundIndex);
const distances = computed(() => store.state.racing.distances);
const racingHorseCount = computed(() => store.state.racing.racingHorseCount);

let isRacing: boolean = false
let isFirstRace: boolean = true
...
// create list for all rounds randomly
const generateRace = () => {
  const roundsTemp = [];
  for (let i = 0; i < distances.value.length; i++) {
    const distance = distances.value[i];
    const horsesRandom = getRandomHorses(racingHorseCount.value);
    roundsTemp.push({
      id: i + 1,
      distance: distance,
      horses: horsesRandom,
  ...
  store.dispatch('racing/fetchRounds', roundsTemp);
};

const runRounds = (index: number) => {
  ...
  let intervalID = setInterval(() => {
    rounds.value[index].horses.forEach((horse: Horse) => {
    ...
    if (rounds.value[index].horses.some((horse: Horse) => horse.location >= (width - 100))) {
      clearInterval(intervalID);

      const horsesTemp = [...rounds.value[index].horses];
      const sortedHorses = horsesTemp.sort((a, b) => b.condition - a.condition);
      rounds.value[index].result = sortedHorses;

      // Check if there are more rounds to run
      if (currentRoundIndex.value < 5) {
        store.dispatch('racing/incrementCurrentRoundIndex');
        runRounds(currentRoundIndex.value);
      } else {
        isRacing = false
        isFirstRace = false
      }
...
const startRace = () => {
  if (!isFirstRace) {
    store.dispatch('racing/resetRace');
  }
  isRacing = true
  runRounds(currentRoundIndex.value)
}
